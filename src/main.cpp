...
void pts_transform(vector<double> &ptsx, vector<double> &ptsy, double px, double py, double psi) {
  for (int i = 0; i < ptsx.size(); i++) {
    ptsx[i] -= px;
    ptsy[i] -= py;
    double x = ptsy[i] * sin(psi) + ptsx[i] * cos(psi);
    double y = ptsy[i] * cos(psi)-ptsx[i] * sin(psi);
    ptsx[i] = x;
    ptsy[i] = y;
  }
}
